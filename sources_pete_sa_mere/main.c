...
#define KDP_S						40			// Differential coefficient

#define KP_V						130		// Proportional coefficient
#define KDP_V						300			// Differential coefficient
...
int servo_base = 7400;														// initial servo position (center)
...
//	PIOB_PDOR &= ~GPIO_PDOR_PDO(1<<18);  // red LED on
	...
	if (BlackLineRight <= 92 && BlackLineLeft <= 12) 	// Left turn case
	{
		servo_position = KP_V*diff + KDP_V*(diff-diff_old);
		TPM0_C1V = 100;					// TPM0 channel1 left Motor 1 In 1 slow forward
		TPM0_C5V = 160;					// TPM0 channel5 right Motor 2 In 2 slow forward
	}
	else if (BlackLineRight >= 108 && BlackLineLeft >= 20)	// right turn case
	{
		servo_position = KP_V*diff + KDP_V*(diff-diff_old);
		TPM0_C1V = 160;					// TPM0 channel1 left Motor 1 In 1 slow forward
	...
	else 
	{
		...
		TPM0_C1V = 200;					// TPM0 channel1 left Motor 1 In 1 fast forward
		TPM0_C5V = 200;					// TPM0 channel5 right Motor 2 In 2 fast forward
	}
