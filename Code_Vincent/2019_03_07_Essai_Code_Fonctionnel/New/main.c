...
//	PIOB_PDOR &= ~GPIO_PDOR_PDO(1<<18);  // red LED on
	...
	image_processing(&diff, &diff_old, &BlackLineLeft, &BlackLineRight);	// image processing : finds the black lines on the left and right, and finds the middle of the track
