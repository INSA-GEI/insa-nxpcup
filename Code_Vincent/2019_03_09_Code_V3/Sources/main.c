...
#define KP_TURN 						15			// Proportional coefficient in turn
#define KDP_TURN 						13			// Differential coefficient in turn
...
int OldLineLeft = 0;					// used in "camera.c"
int OldLineRight = 0;					// used in "camera.c"
int number_edges = 0;				// used in "camera.c"					// number of edges found per line
int edges_final[10];														// array to store the PineScan pixel difference
...
//	PIOB_PDOR &= ~GPIO_PDOR_PDO(1<<18);  // red LED on
	...
	test_image_processing(&BlackLineLeft, &BlackLineRight, &number_edges, edges_final, &OldLineLeft , &OldLineRight);	// image processing : finds the black lines on the left and right, and finds the middle of the track

	middlecalculate(&RoadMiddle, &RoadMiddle_old, &BlackLineLeft, &BlackLineRight, &diff, &diff_old, &number_edges, edges_final);	// calculate the middle of the road
	...
	{
		servo_position = KP_TURN*diff + KDP_TURN*(diff-diff_old);
		TPM0_C1V = 60;					// TPM0 channel1 left Motor 1 In 1 slow forward
	...
	else if (BlackLineRight >= 120 && BlackLineLeft >= 35)	// right turn case
	{
		...
		TPM0_C5V = 60;					// TPM0 channel5 right Motor 2 In 2 slow forward
		
	}
	else if ((BlackLineLeft >= 30 && BlackLineRight <= 100) || number_edges >= 6)	// try to see the end of the race
	{	
		servo_position = KP_STRAIGHT*diff + KDP_STRAIGHT*(diff-diff_old);
		for (i = 0 ; i < 1000000 ; i++);
	...
	{
		servo_position = KP_STRAIGHT*diff + KDP_STRAIGHT*(diff-diff_old);
		TPM0_C1V = 130;					// TPM0 channel1 left Motor 1 In 1 fast forward
		TPM0_C5V = 130;					// TPM0 channel5 right Motor 2 In 2 fast forward
	}
