...
int check_line = 0;		// check if there is a line. If not, we continue to the same direction
...
void middlecalculate (int * RoadMiddle, int * RoadMiddle_old, int * BlackLineLeft, int * BlackLineRight, int * diff, int * diff_old, int * number_edges, int * edges_final)
{

/*	for (i=0; i<= (sizeof(*edges_final) / sizeof(int)); i++)
	{
		if (edges_final[i] != 0)
		{
			check_line = 1;
...
/*
	...
	if (number_edges == 0 || check_line == 0 )
	{
