...
#define	CAM_SI_HIGH_CAMERA_NEAR 			GPIO_SetPinsOutput(GPIOB,(1<<8)) 	// SI on PTB8
#define	CAM_SI_LOW_CAMERA_NEAR				GPIO_ClearPinsOutput(GPIOB,(1<<8)) 	// SI on PTB8*
#define	CAM_CLK_HIGH_CAMERA_NEAR			GPIO_SetPinsOutput(GPIOB,(1<<9))	// CLK on PTB9
#define	CAM_CLK_LOW_CAMERA_NEAR				GPIO_ClearPinsOutput(GPIOB,(1<<9))	// CLK on PTB9
#define	CAM_SI_HIGH_CAMERA_FAR 				GPIO_SetPinsOutput(GPIOB,(1<<10)) 	// SI on PTB8
#define	CAM_SI_LOW_CAMERA_FAR				GPIO_ClearPinsOutput(GPIOB,(1<<10)) // SI on PTB8*
#define	CAM_CLK_HIGH_CAMERA_FAR				GPIO_SetPinsOutput(GPIOB,(1<<11))	// CLK on PTB9
#define	CAM_CLK_LOW_CAMERA_FAR				GPIO_ClearPinsOutput(GPIOB,(1<<11))	// CLK on PTB9
...
#define K_CAMERA_NEAR 						1.0
#define K_CAMERA_FAR						.0
#define CAMERA_FAR_ID						2
#define CAMERA_NEAR_ID						1

// Define thresholds for Camera Black Line recognition
#define THRESHOLD_high				600			// Higher threshold : does not capture noise but may not capture all maximums.
...
public:
	...
	//Methods
	void CAMERA_NEAR_init(void);					//initializes the camera
	void CAMERA_NEAR_capture(void);				//retrieves data from the camera
	void CAMERA_FAR_init(void);					//initializes the camera
	void CAMERA_FAR_capture(void);				//retrieves data from the camera
	void capture(void);
	void init(void);
