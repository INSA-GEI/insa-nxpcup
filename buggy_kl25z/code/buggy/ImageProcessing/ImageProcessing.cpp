...
void ImageProcessing::CAMERA_FAR_init(){
	 ...
	 *  - PORTB 11 en sortie GPIO digitale. Controle le clock de la caméra.
	 */
...
void ImageProcessing:: Actualise_Servo_1_Camera (void){
	...
	// Permet de changer l'angle du servo a
...
void ImageProcessing::compute_data_threshold(void){
	...
	if(number_edges >= 2 && not(finish)){
		...
		CompareData_low += 10;
		if (CompareData_high > THRESHOLD_high) CompareData_high=THRESHOLD_high;
		if (CompareData_low > THRESHOLD_low) CompareData_low=THRESHOLD_low;
	}
