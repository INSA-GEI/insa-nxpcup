...
void debug_init(){
	...
	GPIOB->PSOR = DEBUG_RED_Pin;
	GPIOB->PDDR |= DEBUG_RED_Pin;
	//Initialize the Green LED (PTB19)
	PORTB->PCR[19] = PORT_PCR_MUX(1);
	GPIOB->PSOR = DEBUG_GREEN_Pin;
	GPIOB->PDDR |= DEBUG_GREEN_Pin;
	//Initialize the Blue LED (PTD1) //WARNING : BLUE LED attached to same pin as SW_USER2
	PORTD->PCR[1] = PORT_PCR_MUX(1);
	GPIOD->PSOR = DEBUG_BLUE_Pin;
	GPIOD->PDDR |= DEBUG_BLUE_Pin;
	...
	GPIOC->PSOR = DEBUG_CAM_LED_Pin;
	GPIOC->PDDR |= DEBUG_CAM_LED_Pin;
	...
	GPIOC->PDDR |= DISP_CLK_PIN;
	PORTC->PCR[6] = PORT_PCR_MUX(1);
	GPIOC->PDDR |= DISP_SIN_PIN;
	PORTC->PCR[10] = PORT_PCR_MUX(1);
	GPIOC->PDDR |= DISP_LATCH_PIN;
	DISP_CLK_OFF;
	DISP_SIN_OFF;
	DISP_LATCH_ON;
...
unsigned char debug_getUserSW1(void){
	return (GPIOD->PDIR & (1<<3))? 0 : 1;
}
unsigned char debug_getUserSW2(void){
	return (GPIOD->PDIR & (1<<1))? 0 : 1;
}
...
void lptmr_conf(void){
	...
	NVIC->ICPR[0U] |= (1 << 28);			// clear pending interrupt 28
	NVIC->ISER[0U] |= (1 << 28);			// enable interrupt 28
