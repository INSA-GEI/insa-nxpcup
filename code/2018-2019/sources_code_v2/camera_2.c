...
void test_image_processing (int * BlackLineLeft, int * BlackLineRight, int * edges, int * number_edges, int * edges_final)
{
...
*****************************************************************************************************************************************************/
	...
	while (i <= 126)
	{
		...
		for (j=i-2; j<= i+2; j++)
		{
			if (ImageDataDifference[i] >= ImageDataDifference[j])
			{
				found = 1;
			}
			else
	...
	while (i <= 127)
	{
		...
		{
			edges_final[j] = ImageDataDifference [i];
			j = j+1;
		}
		i=i+1;
	}
