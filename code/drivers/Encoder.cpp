...
void Encoder::init(void){
	...
	// TODO A porter vers les bons registres
//	TPM2->C0SC = 0;
//	TPM2->C1SC = 0;
//	TPM2->C0SC|=TPM_CnSC_CHIE_MASK | TPM_CnSC_ELSA_MASK;					// Configuration of TPM2 channel_0 for ENC_SIG_A1 (p555)
//	TPM2->C1SC|=TPM_CnSC_CHIE_MASK | TPM_CnSC_ELSA_MASK;					// Configuration of TPM2 channel_1 for ENC_SIG_A2 (p555)
...
void Encoder::init_SENS(void){
	...
	//On peut maintenant lire leur valeur dans GPIOx_PDIR (par d�faut les pins sont � 0 = input)
}
...
void Encoder::interruptHandler(void){
	...
	if ((TPM2->SC & TPM_SC_TOF_MASK)) {//Clear the bit flag of the overflow interrupt FTM2
		...
		OVF_cnt1++;
		OVF_cnt2++;
		if(OVF_cnt1>MAX_OVF || OVF_cnt1<MAX_OVF)delta1=0; //la voiture n'avance pas ou tr�s lentement
		if(OVF_cnt2>MAX_OVF || OVF_cnt2<MAX_OVF)delta2=0; //la voiture n'avance pas ou tr�s lentement
	}

	// TODO A porter vers les bons registres

//	if ((TPM2->C0SC & TPM_CnSC_CHF_MASK)) {//Clear the bit flag of the capture1 FTM2
//		TPM2->C0SC |= TPM_CnSC_CHF_MASK ;
//		int ccr1=TPM2->C0V;
//
//		delta1 = ccr1 - prev_ccr1 + OVF_cnt1*ENCODER_MOD;
//		//On regarde le sens
//		if (!((FGPIOA->PDIR>>1) & 0x1)){
//			delta1=-delta1; //ok
//		}
...
//	TODO A porter vers les bons registres
//	if ((TPM2->C1SC & TPM_CnSC_CHF_MASK)) {//Clear the bit flag of the capture2 FTM2
//		TPM2->C1SC |= TPM_CnSC_CHF_MASK ;
//		int ccr2=TPM2->C1V;
//		delta2 = ccr2 - prev_ccr2 + OVF_cnt2*ENCODER_MOD;
//		//On regarde le sens
//		if (((FGPIOA->PDIR>>2) & 0x1)){
//			delta2=-delta2; //ok
//		}
//		//MAJ car le TMP continue de compter cf p(564)
//		prev_ccr2 = ccr2;
