...
void Car::init(float Te,int MODE){
	...
	//Calcul nb it�ration avant le calcul de la vitesse
...
void Car::Set_diff_speed(void){
	...
	if (enable_brake && !(finish)){
		...
		delta_speed=(abs(Vset)*L_ENTRAXE)/(1.0*r+L_ENTRAXE);//On l'aide � tourner
		...
		//Soft turn
		float r=LENGHT_CAR/(abs(servo_angle_moy)*DEG_TO_RAD); //r=radius of the turn
		delta_speed=(abs(Vset)*L_ENTRAXE)/(2.0*r+L_ENTRAXE);//th�orique avec 2.0*r
	}else{
		//hard turn
		float r=LENGHT_CAR/(abs(servo_angle_moy)*DEG_TO_RAD); //r=radius of the turn
		delta_speed=(abs(Vset)*L_ENTRAXE)/(1.5*r+L_ENTRAXE);//th�orique avec 2.0*r
	}
...
void Car::Detect_state(void){
	...
	if (enable_finish && (!reset) && !(finish)){
		if ((cam.number_edges)==NB_LIGNES_FIN && state_turn_car<3){//Nb de bandes noires (+1 pour chaque c�t�)
			C_finish=0;
			finish=true;
...
void Car::Set_deplacement(void){
	...
	myMovement.regulate(); //Applique la PWM correspond � la vitesse aux moteurs
}
...
void Car::Aff_debug(void){
	if(FLAG_SEND_IMG && FLAG_ENABLE_LOG_IMG){
		...
		//uart_write(" ",1);
		uart_write(" / ",3);
		uart_write("V_apply=",8);
		uart_writeNb(V_apply);
		//uart_write(" ",1);
		uart_write(" / ",3);
		uart_write("V_mes=",6);
		uart_writeNb(V_mes);
